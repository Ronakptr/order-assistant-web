...
function toCurrency(value) {
  ...
  return `${toFaNumber(number)} تومان`;
}
...
function formatDateTimeForDisplay(dateValue) {
  const jalaliDate = getJalaliPartsFromInputDate(dateValue);

  const timePart = new Date().toLocaleTimeString("fa-IR", {
    hour: "2-digit",
    minute: "2-digit",
  });

  return `${formatJalaliDate(jalaliDate)} - ${timePart}`;
}
...
function normalizeInvoiceSettings(raw) {
  ...
  return {
    ...
    showLogo:
      visibility?.logo ?? raw?.showLogo ?? DEFAULT_INVOICE_SETTINGS.showLogo,
    ...
    showAddress:
      visibility?.address ?? raw?.showAddress ?? DEFAULT_INVOICE_SETTINGS.showAddress,

    showPhones:
      visibility?.phones ?? raw?.showPhones ?? DEFAULT_INVOICE_SETTINGS.showPhones,

    showWebsite:
      visibility?.website ?? raw?.showWebsite ?? DEFAULT_INVOICE_SETTINGS.showWebsite,

    showEmail:
      visibility?.email ?? raw?.showEmail ?? DEFAULT_INVOICE_SETTINGS.showEmail,
    ...
    showFooter:
      visibility?.footer ?? raw?.showFooter ?? DEFAULT_INVOICE_SETTINGS.showFooter,
  };
}

function loadInvoiceSettings() {
  for (const key of INVOICE_SETTINGS_KEYS) {
    try {
      const raw = localStorage.getItem(key);
      if (!raw) continue;
...
function normalizeRow(row) {
  ...
  return {
    ...
    unitPriceRaw: String(row?.unitPriceRaw || extractNumber(row?.unitPrice) || ""),
    unitPrice: row?.unitPrice || toCurrency(row?.unitPriceRaw || 0),

    total: row?.total || toCurrency(row?.totalRaw || 0),
    totalRaw: Number(row?.totalRaw || 0),
...
function JalaliCalendarPicker({
  ...
  const today = getJalaliPartsFromInputDate(toInputDate(new Date()));
...
function buildInvoiceElement({ order, settings }) {
  ...
  const remaining = Math.max(0, subtotal - prepayment);
...
export default function NewOrder() {
  ...
  const [calendarView, setCalendarView] = useState(() =>
    getJalaliPartsFromInputDate(toInputDate(new Date()))
  );
  ...
  useEffect(() => {
    ...
    const loadOrderForEdit = async () => {
      try {
        ...
        setOrderDate(foundOrder.dateInputValue || toInputDate(new Date()));
  ...
  const resetWholePage = () => {
    ...
    setOrderDate(toInputDate(new Date()));
