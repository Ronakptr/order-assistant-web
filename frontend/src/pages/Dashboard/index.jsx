...
function getReadAlertsStorageKey() {
  const user = getStoredUser();
  const companyId = user?.company_id || user?.companyId || user?.company?.id || "default";
  const userId = user?.id || user?.uid || user?.user_id || user?.username || "user";

  return `${READ_ALERTS_STORAGE_KEY}::company:${companyId}::user:${userId}`;
}

function getStoredReadAlertIds() {
  try {
    const rawValue = localStorage.getItem(getReadAlertsStorageKey());
...
function saveStoredReadAlertIds(ids) {
  localStorage.setItem(getReadAlertsStorageKey(), JSON.stringify(ids));
}

function filterAlertsByNotificationSettings(alerts, notificationSettings, currentUser) {
  const settings = normalizeNotificationSettings(notificationSettings || {});

  return asArray(alerts).filter((alert) => {
    return isAlertAllowedForRole(settings, currentUser?.role, alert?.type || "");
  });
}
...
export default function Dashboard() {
  ...
  const [notificationSettings, setNotificationSettings] = useState(() => normalizeNotificationSettings());
  ...
  const filteredAlerts = useMemo(
    () => filterAlertsByNotificationSettings(summary?.alerts, notificationSettings, currentUser),
    [summary?.alerts, notificationSettings, currentUser]
  );

  const notifications = useMemo(
    () => buildNotifications(filteredAlerts, readAlertIds),
  ...
  const ordersByDay = asArray(charts.orders_by_day).map((item) => ({
    ...item,
    label: transformDateTextForApp(item.label || item.date || "-"),
  ...
  useEffect(() => {
    ...
    window.addEventListener("order-assistant-date-settings-updated", handleActivityUpdate);

    return () => {
      window.removeEventListener("order-assistant-activity-log-updated", handleActivityUpdate);
      window.removeEventListener("order-assistant-date-settings-updated", handleActivityUpdate);
    };
  }, []);
  ...
  useEffect(() => {
    function handleAuthChanged() {
      ...
      setReadAlertIds(getStoredReadAlertIds());
    }
  ...
  useEffect(() => {
    ...
    fetchNotificationSettings()
      .then((data) => {
        if (alive) setNotificationSettings(data);
      })
      .catch(() => {
        if (alive) setNotificationSettings(normalizeNotificationSettings());
      });

    const refreshSettings = () => {
      fetchNotificationSettings()
        .then((data) => setNotificationSettings(data))
        .catch(() => setNotificationSettings(normalizeNotificationSettings()));
    };

    window.addEventListener("order-assistant-notification-settings-updated", refreshSettings);

    return () => {
      alive = false;
      window.removeEventListener("order-assistant-notification-settings-updated", refreshSettings);
    };
  }, [currentUser?.role]);
                    ...
                    recentOrders.map((row) => {
                      ...
                      return (
                        <tr key={row.id || row.order_code}>
                          ...
                          <td>{formatDashboardDate(row.dateInputValue || row.date_input_value || row.date || row.order_date_text || row.created_at || row.createdAt)}</td>
