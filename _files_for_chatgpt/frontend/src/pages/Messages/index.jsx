...
function normalizeOrder(order, index) {
  ...
  return {
    ...
    phone:
      ...
      order?.phoneNumber ||
      "",
    email: order?.email || order?.customerEmail || order?.emailAddress || "",
    total: order?.total || order?.totalPrice || order?.grandTotal || "",
    status: order?.status || "",
...
function normalizeMessage(message, index) {
  return {
    uid: message?.uid || message?.id || Date.now() + index,
    id:
      message?.id ||
      message?.messageCode ||
      `MSG-${String(index + 1).padStart(4, "0")}`,
    orderCode: message?.orderCode || message?.code || "",
    invoiceCode: message?.invoiceCode || message?.fax || message?.orderCode || "",
    customer: message?.customer || message?.name || "مشتری جدید",
    phone: message?.phone || "",
    email: message?.email || "",
    items: Array.isArray(message?.items) ? message.items : [],
    status: message?.status || "پیش نویس",
    template: message?.template || message?.type || "صورتحساب",
    channel: message?.channel || "پیامک",
    date: message?.date || new Date().toLocaleDateString("fa-IR"),
...
function NewMessageForm({
  ...
  const [form, setForm] = useState(() => ({
    ...
    channel:
      initialData?.channel ||
      MESSAGE_TEMPLATES[initialData?.template || "صورتحساب"]?.channel ||
      "پیامک",
        ...
        <div className="msg-form__body">
          ...
          <div className="msg-form__row msg-form__row--two">
            <div className="msg-form__field">
              ...
              >
                {Object.keys(MESSAGE_TEMPLATES).map((templateName) => (
            ...
            {paginatedMessages.length === 0 ? (
              ...
              paginatedMessages.map((message, index) => (
                ...
                >
                  ...
                  <td className="mt-cell">{message.date || "-"}</td>
                </tr>
              ))
            )}
          </tbody>
